...
export default function OrganizationSettingsManager({
  ...
  const updateThemeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: t('settings.theme.updated'),
      });
    },
    onError: (error: Error) => {
      toast({
        title: t('common.error'),
        description: error.message || t('settings.theme.error'),
        variant: "destructive",
      });
    },
  });
  ...
  const updateContactMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t('settings.saved'),
        description: t('settings.contact.updated'),
      });
    },
    onError: (error: Error) => {
      toast({
        title: t('common.error'),
        description: error.message || t('settings.contact.error'),
        variant: "destructive",
      });
    },
  });
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="theme" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="contact">{t('settings.contact.title')}</TabsTrigger>
          <TabsTrigger value="payment">{t('settings.payment.title')}</TabsTrigger>
          <TabsTrigger value="shipping">{t('settings.shipping.title')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="theme" className="space-y-4">
          {themeError ? (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {t('settings.theme.loadError')}: {(themeError as Error).message}
              </AlertDescription>
            </Alert>
          ) : (
            <Card className="p-6">
        ...
        <TabsContent value="contact" className="space-y-4">
          {contactError ? (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                {t('settings.contact.loadError')}: {(contactError as Error).message}
              </AlertDescription>
            </Alert>
          ) : (
            <Card className="p-6">
        ...
        <TabsContent value="payment" className="space-y-4">
          {paymentError ? (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                {t('settings.payment.loadError')}: {(paymentError as Error).message}
              </AlertDescription>
            </Alert>
          ) : (
            <PaymentSettingsForm
        ...
        <TabsContent value="shipping" className="space-y-4">
          {shippingError ? (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                {t('settings.shipping.loadError')}: {(shippingError as Error).message}
              </AlertDescription>
            </Alert>
          ) : (
