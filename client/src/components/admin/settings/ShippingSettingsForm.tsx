...
export default function ShippingSettingsForm({
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || "No se pudo guardar la configuración de envíos.",
        variant: "destructive",
      });
    },
  });
      ...
      <CardContent className="space-y-4">
        <div className="p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-center">
          <p className="text-gray-600 dark:text-gray-400">
            Formulario de configuración de envíos (pendiente de implementación)
          </p>
        ...
        <Button onClick={handleSave} disabled={updateMutation.isPending}>
          {updateMutation.isPending ? "Guardando..." : "Guardar Envíos"}
        </Button>
      </CardContent>
    </Card>
  );
