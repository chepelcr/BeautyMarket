...
export class AWSSecretsService implements IAWSSecretsService {
  ...
  async getDatabaseCredentials(): Promise<DatabaseCredentials> {
    const secretName = "dev/strawberry/db";

    try {
      console.log(
        `🔐 Retrieving database credentials from AWS Secrets Manager: ${secretName}`,
      );
      ...
      if (!result.SecretString) {
        throw new Error(
          "Secret string not found in AWS Secrets Manager response",
        );
      }
      ...
      console.log(
        `✅ Database credentials retrieved successfully from AWS Secrets Manager`,
      );
  ...
  async getSMTPCredentials(): Promise<SMTPCredentials> {
    ...
    try {
      console.log(
        `📧 Retrieving SMTP credentials from AWS Secrets Manager: ${secretName}`,
      );
      ...
      console.log(
        `✅ SMTP credentials retrieved successfully from AWS Secrets Manager`,
      );
      console.log(`   Host: ${credentials.host}`);
      ...
      console.error(
        `❌ Failed to retrieve SMTP credentials from AWS Secrets Manager:`,
        error,
      );
  ...
  private validateDatabaseCredentials(
    credentials: any,
  ): asserts credentials is DatabaseCredentials {
    const required = [
      "username",
      "password",
      "engine",
    ...
    if (credentials.engine !== "postgres") {
      throw new Error(
        `Unsupported database engine: ${credentials.engine}. Expected 'postgres'`,
      );
    }
  ...
  private validateSMTPCredentials(
    credentials: any,
  ): asserts credentials is SMTPCredentials {
