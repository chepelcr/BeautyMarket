...
export function PreDeploymentBanner() {
  ...
  useEffect(() => {
    ...
    const loadActivePreDeployment = async () => {
      try {
        const response = await apiRequest('GET', buildOrgApiUrl(user.id, organization.id, '/pre-deployments/active'));
  ...
  const handlePublish = async () => {
    ...
    try {
      await apiRequest("POST", buildOrgApiUrl(user.id, organization.id, "/deployments"), {});
  ...
  const handleDismiss = async (preDeploymentId: string) => {
    ...
    try {
      await apiRequest("DELETE", buildOrgApiUrl(user.id, organization.id, `/pre-deployments/${preDeploymentId}`), {});
      // Refresh the active pre-deployment status
      const response = await apiRequest('GET', buildOrgApiUrl(user.id, organization.id, '/pre-deployments/active'));
      const data = await response.json();
      setActivePreDeployment(data);
