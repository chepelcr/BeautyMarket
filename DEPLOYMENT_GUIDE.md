...
- **API Gateway**: REST API with custom domain (`api.jmarkets.jcampos.dev`)
- **Cognito**: User authentication and management
- **RDS/Supabase**: PostgreSQL database
- **SES**: Email service for notifications

### Frontend Infrastructure
- **Dashboard** (Admin Panel): React SPA at `admin.jmarkets.jcampos.dev`
- **Landing Page**: Marketing site at `jmarkets.jcampos.dev`
- **Template Sites**: Multiple tenant sites at `{subdomain}.jmarkets.jcampos.dev`
...
├── template.yaml           # Lambda function (SAM)
├── api-gateway.yml         # API Gateway + custom domain
├── static-website.yml      # S3 + CloudFront for frontend
...
- Sets up custom domain (`api.jmarkets.jcampos.dev`)
...
- Landing: `jmarkets.jcampos.dev`
- Templates: `{subdomain}.jmarkets.jcampos.dev`

**Outputs:**
- Landing page URL: `https://jmarkets.jcampos.dev`
- Dashboard URL: `https://admin.jmarkets.jcampos.dev`
- Template URLs: `https://{subdomain}.jmarkets.jcampos.dev`
...
TOKEN=$(curl -X POST https://api.jmarkets.jcampos.dev/api/login \
  -H "Content-Type: application/json" \
  -d '{"email":"user@example.com","password":"Password123!"}' \
  | jq -r '.token')

# Use token in authenticated request
curl https://api.jmarkets.jcampos.dev/api/users/[userId]/profile \
...
```bash
VITE_API_URL=https://api.jmarkets.jcampos.dev
...
```bash
# The setup script automatically creates wildcard cert for *.jmarkets.jcampos.dev
...
dig admin.jmarkets.jcampos.dev
dig api.jmarkets.jcampos.dev
...
Dashboard: `https://admin.jmarkets.jcampos.dev`
API: `https://api.jmarkets.jcampos.dev`
Landing: `https://jmarkets.jcampos.dev`
