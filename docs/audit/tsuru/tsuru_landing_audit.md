...
| SEO defaults | `src/content/seo.json` → `"defaultTitle": {"es": "JMarkets - Tu comunidad, tu mercado"}`, `siteUrl: "https://j-markets.jcampos.dev"` |
...
| 7 | Own online address from day one ("Tu tienda tiene una dirección única desde el primer día") | `features.json` → `featureCards[5]` | Maps to per-org subdomains (`{slug}.j-markets.jcampos.dev`) in the platform. |
...
| 27 | Live store examples to browse ("Ver ejemplos") | `landing.json` → `hero.secondary`; `src/pages/Examples.tsx` | Implemented: fetches real templates from the platform API and links to `https://{name}-example.j-markets.jcampos.dev`. |
...
2. **Deployment-identity drift.** `seo.json` declares `siteUrl: "https://j-markets.jcampos.dev"` and `branding.json` points CTAs at `https://admin.j-markets.jcampos.dev`, while the repo's own `landing-client/CLAUDE.md` states the site is hosted at `https://tsuru.jcampos.dev`. Canonical URLs, sitemap, and OG tags generated from `seo.json` would therefore point at the old domain.
