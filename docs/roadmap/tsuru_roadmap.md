...
1. **Reseed the catalog (owner — blocks the `organization` module + doc-type create gates)**: `cd server && pnpm run db:reseed-rbac`. Migration 0002 was applied 2026-06-11 (org general/theme columns live; General/Theme saves work once the Lambda deploy lands). The reseed (destructive, blocked for automation) activates: the `organization` module + 9 settings submodules, the doc-type submodules `documents/fe|te|nc|nd|fc|fexp`, and the staff restriction (FE/TE only).
