...
| TSR-037 | **RBAC catalog realignment** — seeded catalog now mirrors the POS sidebar 1:1 (modules `panel/documents/commercial/admin/storefront/reports`; submodules = sidebar items; **pos folded into `documents/emitted`**, no separate pos module); `submoduleActionMatrix` + `rolePermissionMatrix` remapped; RBACController guards `team`→`admin`; destructive reseed script `db:reseed-rbac` (preserves roles/memberships, aborts on custom org roles unless `--force`) | **Done** (code, committed) — **live reseed pending (§7 step 1)** | User feedback 2026-06-11 (drawer showed old modules) | `server/src/seeds/rbac-seed.ts` (rewritten), `src/scripts/run-rbac-reseed.ts`; rule documented in `server/CLAUDE.md` + POS `CLAUDE.md` §5.1. Owner must run `cd server && pnpm run db:reseed-rbac` (destructive — blocked for automation). |
...
| Order | Items | Notes |
|---|---|---|
| 1 | ~~TSR-026~~ Done → **TSR-037 live reseed** (`pnpm run db:reseed-rbac`, §7 step 1) | The single remaining DB step; until it runs, the roles drawer serves the old catalog. |
...
| R12 | ~~System `admin` role template can't reach the roles UI under enforce mode~~ **Resolved by TSR-037** (admin template carries `admin:['create','read','update','invite']`) — effective once the catalog reseed runs | Closed (pending reseed) | TSR-035/TSR-037 | Flip TSR-027 only after the reseed. |
...
The earlier list (migrate, seed, commit, mirror, swagger) is **done**: migration + seeds + org-modules backfill applied to the live DB; everything committed and pushed (monorepo `develop`→`main` AND the three standalone repos); the GH Actions Lambda deploy ran green. What actually remains, in order:

1. **Run the catalog reseed (owner — blocks the roles drawer)**: `cd server && pnpm run db:reseed-rbac`. The live DB still holds the OLD module/submodule catalog from the first seed run; the realigned sidebar-mirroring catalog (TSR-037) only exists in code. The script is destructive (deletes and recreates the catalog + grants in FK order; preserves roles/memberships; aborts if custom org roles exist — currently 0, so it's safe) and is therefore blocked for automation — run it from your terminal (e.g. `! cd server && pnpm run db:reseed-rbac` inside a Claude session).
...
3. **API Gateway authorizer verify (TSR-028)**: confirm `/api/admin/*` and the org-scoped `/rbac/*` routes ride the proxy WITH the Cognito authorizer before any hosted landing-admin build.
4. **userId↔JWT-sub mapping at the gateway (TSR-029)** — load-bearing for trusting O1–O15 in production.
5. **Enforce flip (TSR-027)**: after the reseed (step 1) and real use of the POS roles UI, set `RBAC_ENFORCEMENT=enforce`; consider making POS `usePermissions` fail-closed at the same time. TSR-035 is already satisfied by the realigned catalog.
6. **Admin Cognito pool (TSR-039 follow-up)**: build the dedicated pool, then remove `ADMIN_AUTH_MODE=open` / `VITE_ADMIN_AUTH_MODE=open`. Never deploy a hosted admin build while `open` is set.
7. **Landing admin env (private builds only)**: `VITE_API_URL`, Cognito vars, `VITE_ENABLE_ADMIN=true` — never in the public GitHub Pages workflow.
8. **Repo-split mirroring (TSR-100)**: keep mirroring every monorepo change to the standalone repos until TSR-090 untracks the folders.
