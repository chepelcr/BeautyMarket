...
| ID | Item | Status | Source | Evidence / next step |
|---|---|---|---|---|
| TSR-090 | Finish the repo split: migrate CI off monorepo paths, untrack `templates/pos-system` + `landing-client` + `server`, kill the duplicate landing deploy path | In progress | T1, O10, Top-10 #8, Strat §5.1 | All THREE folders now extracted (POS, landing, **server → TSR-104**) but dual-tracked (root CLAUDE.md split rules). With the GH Actions Lambda deploy green, the monorepo CodePipeline backend stages are redundant — disabling them is the next concrete step (§7). |
...
2. **Disable the redundant monorepo CodePipeline backend stages** — the standalone repo's GH Actions flow (TSR-104) now owns Lambda + API Gateway deploys; a monorepo pipeline run could deploy stale code over it.
...
7. **Repo-split mirroring (TSR-100)**: keep mirroring every monorepo change to the standalone repos until TSR-090 untracks the folders.
