...
1. **Apply migration 0002 + reseed the catalog (owner — blocks org-settings saves + the new organization module)**: `cd server && pnpm run db:run-migration migrations/0002_thin_nomad.sql && pnpm run db:reseed-rbac`. Until the migration runs, the deployed General/Theme saves fail (missing columns); until the reseed runs, the `organization` module is absent and its grants/card gating don't apply. Both blocked for automation (live shared Postgres).
