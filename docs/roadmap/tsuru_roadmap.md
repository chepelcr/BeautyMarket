...
1. **Disable the redundant monorepo CodePipeline backend stages** — the standalone repo's GH Actions flow (TSR-104) now owns Lambda + API Gateway deploys; a monorepo pipeline run could deploy stale code over it.
